...
bagitem['duration'].each do |n|
	if platform?('redhat')
		execute "Memory Load at #{bagitem['utilization_level'].at(2)}% for #{n}mins (redhat)" do
			command <<-EOS
			cd #{ENV['HOME']}
		...
		log "after test log" do
			message "Finished executing Memory Load at #{bagitem['utilization_level'].at(2)}% for #{n}mins"
			level :info
		end
		...
		powershell_script "Memory Load at #{bagitem['utilization_level'].at(2)}% for #{n}mins (windows)" do
			code <<-EOS
			cd #{ENV['HOME']}
